...
*
      ...
      INTEGER, save :: TEMP_NCASE(MAX_TEMP)
      INTEGER, save :: TEMP_ICODE(MAX_TEMP)
      Real*4, save :: TEMP_ETRACK(MAX_TEMP)
      
      
      
      
      
      !Patient/Operator/shield variables
      REAL*4, save :: EFINAL(MAX_TEMP)
      REAL*4, save :: TEMP_XPOS(MAX_TEMP)
      REAL*4, save :: TEMP_YPOS(MAX_TEMP)
      REAL*4, save :: TEMP_ZPOS(MAX_TEMP)
      INTEGER, save :: TEMP_MREG(MAX_TEMP)
      
      
      
      ! Crossing variables
       !Real*4, save :: TEMP_ETRACK_CROSS(MAX_TEMP)
       !REAL*4, save :: CROSS_XPOS(MAX_TEMP)
       !Real*4, save :: CROSS_YPOS(MAX_TEMP)
       !Real*4, save :: CROSS_ZPOS(MAX_TEMP)
       !INTEGER, save :: CROSS_MREG(MAX_TEMP)
...
*                                                                      *
     ...
     & TEMP_ZPOS(I), TEMP_MREG(I), 'Op_cross' !Writing the crossing details. -9999.0 is a placeholder since the crossing does not have a final energy 
            ...
            ELSE
            !It is an interaction event
         
               IF (TEMP_MREG(I) > 3) THEN !The MREG region belongs to the patient, writing out interactions inside the patient
                  WRITE (98, *) TEMP_NCASE(I), TEMP_ETRACK(I),
     & EFINAL(I), TEMP_ICODE(I), TEMP_XPOS(I), TEMP_YPOS(I), 
     & TEMP_ZPOS(I), TEMP_MREG(I), 'Patient' 
               ELSE IF (TEMP_MREG(I) .EQ. 3) THEN !The MREG region belongs to the operator, writing out interactions inside the operator
                  WRITE (98, *) TEMP_NCASE(I), TEMP_ETRACK(I),
     & EFINAL(I), TEMP_ICODE(I), TEMP_XPOS(I), TEMP_YPOS(I),
...
*
      ...
      IF (MREG .NE. 3 .AND. NEWREG .EQ. 3) THEN !We have a crossing from a none operator region into operator
